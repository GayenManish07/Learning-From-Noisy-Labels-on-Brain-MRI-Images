...
class CoCorrecting(BasicTrainer, Loss):
    ...
    def __init__(self, args):
        ...
        #if os.path.isfile(self.args.checkpoint_dir):           #modified to bypass loading from checkpoint as it was causing errors
         #   self._resume()
        #else:
    ...
    def _cluster_data_into_subsets(self):
        ...
        cc = CurriculumClustering(n_subsets=3, verbose=True, random_state=0, dim_reduce=50) # '''self.args.dim_reduce''')
...
def accuracy(output, target, topk=(1,)):
    ...
    pred = pred.t()
    correct = pred.eq(target.reshape(1, -1).expand_as(pred))    #changed view to reshape
